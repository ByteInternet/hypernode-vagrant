...
class TestRunVagrantUp(TestCase):
    ...
    def test_run_vagrant_up_starts_vagrant_in_shell_in_specific_directory_without_provisioning_if_specified(self):
        run_vagrant_up('/tmp/tmpdir1234', no_provision=True)

        self.run_local_command.assert_called_once_with(
            'cd /tmp/tmpdir1234 && vagrant up --no-provision',
            shell=True
        )
