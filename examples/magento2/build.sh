...
echo "Starting a hypernode-vagrant and installing Magento 2"

TEST_COMMAND="bash scripts/build.sh"
TEST_USER='app'  # Other options: app, root, vagrant (can sudo while app can not)
...
PYTHONPATH=${HYPERNODE_VAGRANT_RUNNER_DIR}/tools/hypernode-vagrant-runner \
    ...
    --pre-existing-vagrant-path="$PROJECT_DIRECTORY/hypernode-vagrant" \
